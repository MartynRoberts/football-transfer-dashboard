...
async function main() {
  ...
  for (const [index, player] of players.entries()) {
    ...
    try {
      console.log(`${progress} Syncing injuries for ${player.name}`);
      const synced = await syncPlayerInjuries(
        player.id,
      ...
      if (synced) {
        ...
        failed++;
        console.warn(`${progress} No valid injury response for ${player.name}`);
      }
    } catch (error) {
      failed++;
  ...
  console.log(`Injuries complete: ${succeeded} succeeded, ${failed} failed`);
...
main()
  .catch((error) => {
    console.error("Fatal injury sync error:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
