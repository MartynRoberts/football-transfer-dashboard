...
export default function BestValueTransfers({
  transfers,
  season,
}: {
  transfers: BestValueTransfer[];
  season: string;
}) {
  return (
    <section>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {season} — biggest difference between valuation and fee paid.
        </p>
      </div>

      {transfers.length === 0 ? (
        <p className="text-sm text-slate-500">
          No comparable current-season transfers available.
        </p>
      ) : (
          ...
          <table className="w-full border">
            <thead>
              <tr className="border-b bg-gray-100">
                ...
                <th className="p-3 text-left">Value gained</th>
            ...
            <tbody>
              {transfers.map((transfer, index) => (
                <tr key={transfer.id} className="border-b">
                  ...
                  <td
                    data-label="Value gained"
                    className="whitespace-nowrap p-3 font-semibold text-emerald-700"
                  >
                    Worth {formatPounds(transfer.valueDifference)} more than paid
                  </td>
